document.addEventListener("DOMContentLoaded", () => {
 ...
 if(window.location.href.includes("bucherverwaltung.html")){
                     ...
                     data.books.forEach(book => {
                         tableContent += `
                             <tr>
                                 <td>${book.titel}</td>
                                 <td>${book.autor}</td>
                                 <td>${book.status_text} <br /> <span class="tage-bis-rueckgabe">${book.tage_bis_rueckgabe}</span></td>
     ...
     // Überprüfen, welche Seite aktuell ist
     const currentPage = page || 1;
     
     for (let i = 1; i <= totalPages; i++) {
         // Wenn die aktuelle Seite ist, füge die Klasse 'active' hinzu
